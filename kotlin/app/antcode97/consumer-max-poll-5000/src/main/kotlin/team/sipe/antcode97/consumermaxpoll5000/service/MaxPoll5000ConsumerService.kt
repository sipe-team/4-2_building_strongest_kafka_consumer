package team.sipe.antcode97.consumermaxpoll5000.service
...
/**
 * max.poll.records = 5000 카프카 컨슈머 서비스
 * 
 ...
 */
@Service
class MaxPoll5000ConsumerService {
    ...
    companion object {
        ...
        const val GROUP_ID = "max-poll-5000-consumer-group"
    }
    
    /**
     * max.poll.records = 5000 설정으로 메시지 컨슘
     * 
    ...
    @KafkaListener(
        topics = [TOPIC_NAME],
        groupId = GROUP_ID,
        containerFactory = "maxPoll5000KafkaListenerContainerFactory"
    )
    fun consumeUuidMessage(
        ...
        try {
            ...
            if (currentCount % 1000 == 0L) {
                ...
                // 성능 통계 출력
                logger.info("=== max.poll.records=5000 성능 통계 ===")
                ...
                logger.info("설정: max.poll.records=5000 (대량 처리) - 높은 처리량, 배치 처리")
                logger.info("========================")
            }
            ...
            logger.error(
                "[max.poll.records=5000] 메시지 처리 실패 - uuid={}, topic={}, partition={}, offset={}",
